 ...
 * The values of the vertices are validate using the given validation
 * rule.
 *
 * @author Vladimir Kutuev
 */
public class SetVertexValidator extends VertexValidator {

	private static final Logger LOG = LogManager.getLogger(SetVertexValidator.class);
	private static final long MAX_PRINT_ERROR_COUNT = 100;

	final private Path outputPath;
	final private Path validationFile;
	final private ValidationRule<LongSet> rule;
	final private boolean verbose;

	public SetVertexValidator(Path outputPath, Path validationFile, ValidationRule<LongSet> rule, boolean verbose) {
	...
	public boolean validate() throws ValidatorException {
		Long2ReferenceMap<LongSet> validationResults, outputResults;
		...
		for (Long id: keys) {
			String error = null;
			LongSet outputValue = outputResults.get(id);
			LongSet correctValue = validationResults.get(id);
	...
	private Long2ReferenceMap<LongSet> parseFileOrDirectory(Path filePath) throws IOException {
		LOG.info(String.format("Parsing file/directory %s.", filePath));

		final Long2ReferenceMap<LongSet> results = new Long2ReferenceOpenHashMap<>();
				...
				try(BufferedReader reader = new BufferedReader(new FileReader(file.toFile()))) {
					...
					while ((line = reader.readLine()) != null) {
						...
						try {
							...
							LongSet vertexValue = rule.parse(parts.length > 1 ? parts[1] : "");
